...
class CreateVibrationAnalysis:
    ...
    def load_from_outcar(cls, outcar_file:str='OUTCAR', exclude_imag=False)->list[VibrationalMode]:
        ...
        if i_index == 0:
            raise ValueError("Vibrational analysis section not found in OUTCAR")
...
def save_xsf(filename:str, vibmode:VibrationalMode, scale:float=1.0)->int:
    """
...
"""
    ...
    # 全コードを結合
    full_code = header + materials + functions + atoms_def + vibrations_def + draw_commands + bonds + footer
