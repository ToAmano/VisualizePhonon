...
def main():
    ...
    gen_parser = subparsers.add_parser('generate', help='read data from VASP OUTCAR and extract phonon modes.')
    gen_parser.add_argument('--input', '-i', type=str, help='input data. VASP OUTCAR is supported.')
    # gen_parser.add_argument('--output', '-o', type=str, default='phonon_visualization.asy', help='出力Asymptoteファイル')
    gen_parser.add_argument('--format', '-f', type=str, default='xsf', choices=['xsf', 'xyz', 'asy'], help='Output data format. default to xsf')
    gen_parser.add_argument('--mode', '-m', type=int, default=-1, help='The vibration mode. -1 for all modes. default to -1.')
    gen_parser.add_argument('--scale', '-s', type=int, default=1, help='Scale factor of the eigenvector. default to 1.')
    ...
    if args.command == 'generate':
        ...
        if args.mode == -1: # for all mode
            ...
            if args.format == "xsf":
                save_xsf(f"mode_{args.mode}.xsf",analysis.modes[args.mode],args.scale)
            elif args.format == "xyz":
                save_xyz(f"mode_{args.mode}.xyz",analysis.modes[args.mode],args.scale)
            elif args.format == "asy":
                generate_asymptote_phonon_code(analysis.modes[args.mode], f"mode_{args.mode}.tex")
